...
use window_support::{
    checksum_stack_window_runs_par, domain_from_bounds, window_count, window_lens,
};

fn bench_window_par_iter_height_runs(c: &mut Criterion) {
    let mut group = c.benchmark_group("window_par_iter_height_runs");
        ...
        for (case, bounds) in cases(n) {
            ...
            for len in window_lens(from, to) {
                ...
                group.bench_function(BenchmarkId::new("int_interval_stack", id), |b| {
                    b.iter(|| {
                        black_box(checksum_stack_window_runs_par(
                            black_box(&stack),
                            black_box(from),
                            black_box(to),
                            black_box(len),
...
criterion_group! {
    ...
    targets = bench_window_par_iter_height_runs
}
