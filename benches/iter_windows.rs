...
use window_support::{checksum_stack_window_bounds, domain_from_bounds, window_count, window_lens};

fn bench_iter_windows(c: &mut Criterion) {
    let mut group = c.benchmark_group("iter_windows");
        ...
        for (case, bounds) in cases(n) {
            ...
            for len in window_lens(from, to) {
                ...
                group.bench_function(BenchmarkId::new("int_interval_stack", id), |b| {
                    b.iter(|| {
                        black_box(checksum_stack_window_bounds(
                            black_box(&stack),
                            black_box(from),
                            black_box(to),
                            black_box(len),
